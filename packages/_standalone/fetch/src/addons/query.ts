import { urlencode } from "./_utils"
import { FetchAddon } from "../types"
...
function defaultSerialize(query: Record<string, unknown>, url: string) {
    let params = urlencode(query)
    return url + (url.includes("?") ? "&" : "?") + params.toString()
}

export function query(options: QueryAddonOptions = {}): FetchAddon<QueryAddon, object> {
    ...
    return {
        beforeRequest: (ctx) => {
            if (ctx.options.query || ctx.baseOptions.query) {
                let obj =
                    ctx.baseOptions.query && ctx.options.query
                        ? {
