...
import { parseImportSpecifier } from "./external-libs"
...
export function parseJsrJson(json: string): JsrJson {
    ...
    let obj = res.config as Record<string, unknown> | null
    ...
    if (typeof obj.exports !== "string") {
        for (let key in obj.exports as Record<string, unknown>) {
            if (typeof key !== "string") {
                throw new TypeError("exports: Expected key to be a string")
            }

            if (typeof (obj.exports as Record<string, unknown>)[key] !== "string") {
    ...
    if (obj.imports !== undefined) {
        ...
        let imports = obj.imports as Record<string, unknown>
        for (let key in imports) {
            ...
            imports[key] = parseImportSpecifier(imports[key] as string)
        }
    }
    ...
    return obj as unknown as JsrJson
}

let _findClosestJsrJsonCache = new Map<string, string | null>()

export function findClosestJsrJson(path: string): string | null {
    while (true) {
        if (_findClosestJsrJsonCache.has(path)) {
            return _findClosestJsrJsonCache.get(path) ?? null
        }
        for (let file of ["jsr.json", "deno.json", "jsr.jsonc", "deno.jsonc"]) {
