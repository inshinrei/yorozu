...
- `requestIdle(fn, opts?)` — `IdleHandle` with `cancel()`. Uses `requestIdleCallback` when present (`opts.timeout` forwarded; `timeout <= 0` is sent as `1` because rIC ignores non-positive deadlines). Else `setTimeout(fn, timeout ?? 0)` with `didTimeout: true` and `timeRemaining() === 0`. Not on the `timers` namespace.
...
- `createPriorityWorkQueue(opts?)` — in-memory work scheduler. Lanes `WorkPri`: `visible` > `preload` > `background` (FIFO inside a lane; queued re-enqueue of the same id moves to the tail of the new lane). Default concurrency 3 when omitted; otherwise `Math.floor` then floor at 1 (non-finite → 1). `enqueue` returns `false` while that id is **running** (does not replace `run`); queued same-id updates `pri`/`run`. `cancel(id)` drops queued or `abort()`s running (`true` if either). Running cancel does not free the slot until `run` settles — `run` must honor `signal`. `cancelAll()` drops queued and aborts all running. `pause()` / `resume()` stop and restart the pump without aborting running jobs (already-paused pause / not-paused resume are no-ops). `isRunning(id)` is true only while that id is executing; `isBusy(id)` is queued or running. `stats`: `active`, `queued`, `maxActive` (high-water). `onError?(error, id)` for non-abort failures (`AbortError` is silent; other throws go through `unknownToError`). `onError` runs after the id is no longer busy, so same-id retry from `onError` is accepted; host `onError` throws are swallowed. Run errors are never thrown to `enqueue`.
- `createBitmapWorkQueue(opts?)` — bitmap decode scheduler. Lanes `BitmapPri`: `visible` | `preload` only (FIFO inside a lane; queued same-id moves to the new lane with updated `pri`/`source`/`run`). Default concurrency **1** when omitted; otherwise `Math.floor` then floor at 1 (non-finite → 1). Default `idle: true`: preload waits for `requestIdle` before taking a slot (idle-waiters count as `queued`, not `active`); visible bypasses idle. `idle: false` starts preload without waiting. `enqueue` returns `false` while that id is **running**. `pause()` blocks new starts (including visible) without aborting running jobs; `resume()` pumps. `cancel(id)` drops queued (cancels idle handle) or `abort()`s a running id and drops it from `active` immediately so same-id `enqueue` is accepted; a stale decode closes the bitmap and skips `run`. Decodes via `createImageBitmap(source, { signal })`; optional host `run({ signal, bitmap })` owns the bitmap (no `close`); without `run`, bitmap is closed after decode. `onError?(error, id)` matches the network queue for non-abort decode/`run` failures (`AbortError` silent; other throws via `unknownToError`; host `onError` throws swallowed; runs after the id is no longer in `active`).
