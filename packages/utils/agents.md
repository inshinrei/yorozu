...
- `requestIdle(fn, opts?)` — `IdleHandle` with `cancel()`. Uses `requestIdleCallback` when present (`opts.timeout` forwarded). Else `setTimeout(fn, 0)` with `didTimeout: true` and `timeRemaining() === 0`. Not on the `timers` namespace.
