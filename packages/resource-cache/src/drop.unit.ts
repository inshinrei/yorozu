...
describe("dropStripBlob", () => {
    ...
    it("strips with one getMany and one putMany and skips already-stripped rows", async () => {
        let col = await filesCol()
        await col.put({ key: "a", storedAt: 1, bytes: 8, blob: blobOf(8), meta: {} })
        await col.put({ key: "z", storedAt: 2, bytes: 0, meta: {} })
        let get = vi.spyOn(col, "get")
        let put = vi.spyOn(col, "put")
        let getMany = vi.spyOn(col, "getMany")
        let putMany = vi.spyOn(col, "putMany")
        await dropStripBlob().apply(col, { keys: ["a", "z"], reason: "bytes" })
