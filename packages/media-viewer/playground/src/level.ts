...
export let LEVEL_KEY: string = "yorozu-media-viewer-level"
...
function readStored(): AnimationLevel {
    if (typeof localStorage === "undefined") return defaultAnimationLevel(prefersReducedMotion())
...
export function setAnimationLevel(next: AnimationLevel): void {
    ...
    current = next
    if (typeof localStorage !== "undefined") localStorage.setItem(LEVEL_KEY, next)
    applyDocument(next)
